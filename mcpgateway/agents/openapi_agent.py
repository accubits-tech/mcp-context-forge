...
class OpenAPIAgent:
    ...
    def __init__(self, llm_service: Optional[LLMService] = None):
        ...
        if not self.llm_service.is_configured():
            logger.warning("LLM service is not configured. Set LLM_API_KEY environment variable to enable AI-enhanced analysis.")

    def _is_ai_available(self) -> bool:
        """Check if AI-enhanced analysis is available."""
        return self.llm_service.is_configured()

    async def analyze_openapi_spec(self, spec: Dict[str, Any], context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
...
Return your response as a valid JSON object with the above sections as keys."""
    ...
    def _generate_basic_analysis(self, spec: Dict[str, Any], context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        endpoint_count = len([1 for path_item in paths.values() for method in path_item.keys() if method.upper() in ["GET", "POST", "PUT", "DELETE", "PATCH"]])
        ...
        return {
            ...
            "authentication_analysis": {"schemes": list(security_schemes.keys()), "global_security": bool(global_security), "scheme_count": len(security_schemes)},
            "parameter_mapping": {},
            "tool_generation_recommendations": {
                ...
                "authenticated_endpoints": len(
                    [
                        ...
                        for method, operation in path_item.items()
                        if method.upper() in ["GET", "POST", "PUT", "DELETE", "PATCH"] and (operation.get("security") or global_security)
                    ]
                ),
            },
            "metadata": {"generated_by": "basic_analysis", "ai_enhanced": False},
        }

    async def enhance_tool_descriptions(self, tools: List[Dict[str, Any]], analysis: Dict[str, Any]) -> List[Dict[str, Any]]:
...
Focus on making the tools more discoverable and easier to understand for end users."""
    ...
    async def validate_security_configuration(self, spec: Dict[str, Any], tools: List[Dict[str, Any]]) -> Dict[str, Any]:
...
Return as a valid JSON object."""
    ...
    def _generate_basic_security_analysis(self, spec: Dict[str, Any], tools: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        return {
            ...
            "authentication_mapping": {"tools_by_auth_type": auth_summary, "total_tools": len(tools), "secured_tools": len([t for t in tools if t.get("requires_auth", False)])},
            "security_risks": ["Basic analysis: Manual security review recommended"],
            "recommendations": ["Review authentication configuration manually", "Verify SSL/TLS usage for production", "Test authorization for all endpoints"],
            "tool_specific_security": {},
            "metadata": {"generated_by": "basic_security_analysis", "ai_enhanced": False},
        }

    async def generate_comprehensive_analysis(self, spec: Dict[str, Any], context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    async def analyze_api_documentation(self, doc_structure: Dict[str, Any], context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
...
Ensure "extracted_endpoints" contains BOTH the regex-found endpoints (if valid) AND any newly discovered ones from the raw text."""
    ...
    def _generate_basic_doc_analysis(self, doc_structure: Dict[str, Any], context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        for param in doc_structure.get("parameters", []):
            ...
            if param_name:
                parameter_analysis[param_name] = {"type": param.get("type", "query"), "description": param.get("description", f"Parameter: {param_name}"), "confidence": 6}
        ...
        return {
            ...
            "metadata": {"generated_by": "basic_doc_analysis", "ai_enhanced": False, "source_format": source_format},
        }
    ...
    def _infer_endpoint_purpose(self, method: str, path: str) -> str:
        ...
        """
        method_purposes = {"GET": "Retrieve", "POST": "Create or process", "PUT": "Update or replace", "DELETE": "Delete", "PATCH": "Partially update"}
    ...
    async def enhance_tools_from_documentation(self, tools: List[Dict[str, Any]], analysis: Dict[str, Any], doc_structure: Dict[str, Any]) -> List[Dict[str, Any]]:
...
Focus on making tools more discoverable, understandable, and safe to use."""
            ...
            if isinstance(response, list):
                ...
                logger.warning(
                    f"AI did not return expected format (type={type(response).__name__}, keys={list(response.keys()) if isinstance(response, dict) else 'N/A'}), returning basic enhanced tools"
                )
                return self._enhance_tools_basic(tools, analysis, doc_structure)
    ...
    def _enhance_tools_basic(self, tools: List[Dict[str, Any]], analysis: Dict[str, Any], doc_structure: Dict[str, Any]) -> List[Dict[str, Any]]:
...
"""
    ...
    def _validate_and_normalize_tool(self, tool: Dict[str, Any], base_url: str) -> Optional[Dict[str, Any]]:
        ...
        for param in tool.get("parameters", []):
            ...
            input_schema["properties"][param_name] = {"type": param_type, "description": param_desc}
        ...
        if method in ["POST", "PUT", "PATCH"] and "body" not in input_schema["properties"]:
            ...
            input_schema["properties"]["body"] = {"type": "object", "description": body_desc}
